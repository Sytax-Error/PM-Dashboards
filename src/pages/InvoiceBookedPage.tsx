...
export default function InvoiceBookedPage() {
    ...
    if (projectId) {
      const fetchBookedInvoices = async () => {
        ...
        try {
          ...
          if (!apiResult.data)
            throw new Error("Failed to fetch booked invoices.");
          ...
          const mapped = apiResult.data.map((item: any, idx: number) => ({
            ...
            invoiceDate: item.invoiceDate
              ...
              : "N/A",
            glDate: item.glDate
              ? new Date(item.glDate).toLocaleDateString("en-IN")
              : "N/A",
  ...
  const filteredData = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter(
        (d) =>
          d.invoiceRefNumber.toLowerCase().includes(s) ||
      ...
      {error && (
        <div className="p-20 flex flex-col items-center justify-center text-center pm-card rounded-2xl">
          ...
          <h3 className="text-lg font-bold text-slate-900">
            Failed to load booked invoices
          </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="pm-card rounded-2xl p-5">
              ...
              <p className="text-2xl font-bold text-red-500 mt-1">
                {filteredData.filter((d) => d.amountPaid === 0).length}
              </p>
            </div>
          </div>
            ...
            {paginatedData.length === 0 && !loading ? (
              <div className="p-20 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  There are no booked invoice records matching the current
                  filters.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-orange-600 text-white">
                      ...
                      <th className="px-4 py-3.5 text-left font-bold text-xs">
                        Inv Type
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginatedData.map((d) => (
                      <tr
                        key={d.srNo}
                        className={`hover:bg-orange-50/40 transition-colors ${d.srNo % 2 === 0 ? "bg-white" : "bg-slate-50/30"}`}
                      >
                        ...
                        <td className="px-4 py-3.5 text-right border-l border-slate-200/60 tabular-nums font-bold text-orange-700">
                        ...
                        <td className="px-4 py-3.5 text-right border-l border-slate-200/60 tabular-nums text-red-600">
                          {d.unpaid > 0 ? formatCurrency(d.unpaid) : "-"}
                        </td>
