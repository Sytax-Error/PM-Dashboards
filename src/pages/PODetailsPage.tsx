...
export default function PODetailsPage() {
      ...
      const fetchPODetails = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `${API_BASE_URL}/PM/polist/project/${projectId}`,
          );
          ...
          const mapped = apiResult.data.map((item: any, idx: number) => ({
            ...
            poDate: item.poDate
              ...
              : "N/A",
            startDate: item.frDate
              ? new Date(item.frDate).toLocaleDateString("en-IN")
              : "N/A",
            endDate: item.toDate
  ...
  const projects = useMemo(
    () => [...new Set(poData.map((d) => d.projectNumber))],
    [poData],
  );
  ...
  const filteredData = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter(
        (d) =>
          d.poWoNumber.toLowerCase().includes(s) ||
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="pm-card rounded-2xl p-5">
              <p className="text-sm text-slate-500">Total POs/WOs</p>
            ...
            {paginatedData.length === 0 && !loading ? (
              <div className="p-20 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  There are no purchase order records matching the current
                  filters.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginatedData.map((d) => (
                      <tr
                        key={d.srNo}
                        className={`hover:bg-green-50/40 transition-colors ${d.srNo % 2 === 0 ? "bg-white" : "bg-slate-50/30"}`}
                      >
                        ...
                        <td className="px-4 py-3.5 text-right border-l border-slate-200/60 tabular-nums font-bold text-green-700">
                          {formatCurrency(d.totalPoAmount)}
                        </td>
                        <td className="px-4 py-3.5 text-center border-l border-slate-200/60">
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${
                              d.approvalStatus === "DISPATCHED"
                  ...
                  <tfoot>
                    <tr className="bg-green-50 font-bold border-t-2 border-green-200">
                      ...
                      <td className="px-4 py-3.5 text-right text-slate-900 border-l border-slate-200/60 tabular-nums">
                        {formatCurrency(totalAmount)}
                      </td>
                      <td colSpan={2}></td>
                    </tr>
