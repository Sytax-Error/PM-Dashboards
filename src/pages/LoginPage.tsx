...
const features = [
  ...
  },
  {
    icon: (
      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m1.636 6.364l.707-.707M12 21v-1m-6.364-1.636l.707-.707M9 12a3 3 0 116 0 3 3 0 01-6 0z" />
      </svg>
    ),
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!password || password.length < 3) { setError("Password must be at least 4 characters."); return; }
  ...
  return (
    <main className="relative min-h-dvh overflow-y-auto bg-[#08111f] text-slate-900">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        />
        <motion.div
          animate={{ opacity: [0.18, 0.38, 0.22, 0.18], rotate: [0, 12, -8, 0] }}
        ...
        >
          ...
          <div className="relative hidden lg:flex flex-col justify-between min-h-[640px] p-10 xl:p-12 text-white">
            ...
            >
              ...
              <div>
                <p className="text-base font-bold leading-tight">PM Dashboard</p>
                <p className="text-xs text-slate-400">NICSI · Payment Monitoring System</p>
              </div>
            </motion.div>
            ...
            >
              ...
              <h1 className="text-3xl xl:text-4xl font-bold leading-tight text-white">
                NICSI Project<br />Management Dashboard
              </h1>
            ...
            >
              <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.28em] text-cyan-300/80">Key Features</p>
              <div className="grid grid-cols-2 gap-2.5">
                {features.map((f, i) => (
                  ...
                  >
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${f.bg} ${f.color}`}>
                      {f.icon}
                    </span>
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-white leading-snug">{f.title}</p>
                      <p className="mt-0.5 text-[11px] text-slate-400 leading-snug">{f.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <div className="flex items-center justify-center bg-white/95 backdrop-blur px-6 py-10 sm:px-10 lg:rounded-r-[2rem]">
            ...
            >
              ...
              <motion.p variants={fadeUp} className="mt-6 text-center text-xs text-slate-400">
                © {new Date().getFullYear()} NICSI · Government of India. All rights reserved.
              </motion.p>
            </motion.div>
          </div>
        </motion.section>
