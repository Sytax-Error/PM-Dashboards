    ...
    if (projectId) {
      const fetchTaxInvoices = async () => {
        ...
        try {
          ...
          const response = await fetch(`${API_BASE_URL}/pm/tax-invoice/project/${projectId}`);
    ...
    <div className="min-h-full flex flex-col gap-6">
      ...
      {error && (
        <div className="p-20 flex flex-col items-center justify-center text-center pm-card rounded-2xl">
          ...
          <h3 className="text-lg font-bold text-slate-900">Failed to load tax invoice details</h3>
          ...
          <div className={`pm-card rounded-2xl overflow-hidden relative ${loading ? "opacity-60" : ""}`}>
            ...
            {paginatedData.length === 0 && !loading ? (
              <div className="p-20 flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-lg font-bold text-slate-900">No tax invoices found</h3>
                <p className="text-sm text-slate-500 mt-1">There are no tax invoice records matching the current filters.</p>
              </div>
            ) : (
