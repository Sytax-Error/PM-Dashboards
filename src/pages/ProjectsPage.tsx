...
function ClickableCell({ value, onClick, color = "blue" }: ClickableCellProps) {
  const colorClasses = {
    green:
      "bg-green-100/50 text-green-700 hover:bg-green-100 cursor-pointer font-bold underline decoration-dotted underline-offset-2",
    ...
    purple:
      "bg-purple-100/50 text-purple-700 hover:bg-purple-100 cursor-pointer font-bold underline decoration-dotted underline-offset-2",
    orange:
      "bg-orange-100/50 text-orange-700 hover:bg-orange-100 cursor-pointer font-bold underline decoration-dotted underline-offset-2",
  };
  ...
  return (
    <td
      ...
      className={`px-4 py-3 text-right border-l border-slate-200/60 tabular-nums select-none transition-colors ${
        value > 0 ? colorClasses[color] : "text-slate-400 cursor-default"
      }`}
    >
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        const total =
          apiResult.totalRecords || filterCount || apiResult.data.length;
        ...
        const mappedProjects = apiResult.data.map((p: any, index: number) => ({
          ...
          projectType:
            p.prjType || p.prjTypCode || filterProjectType || "Unknown",
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Click on highlighted numbers to view detailed records
            </p>
          </div>
        </div>
      ...
      {(filterProjectType || Object.values(columnFilters).some((v) => v)) && (
        <div className="flex items-center gap-3 rounded-2xl border border-primary-200 bg-primary-50 px-5 py-3 text-sm">
          ...
          <span className="text-primary-700">
            Showing <span className="font-bold">{filteredProjects.length}</span>{" "}
            projects with active column filters.
          </span>
          <button
            type="button"
            onClick={onClearFilter}
          ...
          {paginatedProjects.length === 0 && !loading ? (
            <div className="p-20 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-slate-500 mt-1">
                There are no project records matching the current filters.
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-800 text-white border-b border-slate-700">
                    <th className="px-4 py-3.5 text-left font-bold text-xs uppercase">
                      Sr.No.
                    </th>
                    <th className="px-4 py-3.5 text-left font-bold text-xs uppercase">
                      Project No
                    ...
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 uppercase">
                      Budget
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 uppercase">
                      Received
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-green-700 uppercase">
                      PO Count
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-green-700 uppercase">
                      PO Amt
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-blue-700 uppercase">
                      Billdesk
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-purple-700 uppercase">
                      Tax Count
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-purple-700 uppercase">
                      Tax Amt
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-orange-600 uppercase">
                      Booked
                    </th>
                    <th className="px-4 py-3.5 text-right font-bold text-xs border-l border-white/10 bg-orange-600 uppercase">
                      Inv Amt
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginatedProjects.map((project) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-primary-700 font-bold">
                      ...
                      <ClickableCell
                        value={project.noOfPOs}
                        color="green"
                        onClick={() =>
                          navigate(`/projects/${project.projectId}/po`)
                        }
                      />
                      ...
                      <ClickableCell
                        ...
                        onClick={() =>
                          navigate(
                            `/projects/${project.projectId}/invoice-received`,
                          )
                        }
                      />
                      ...
                      <ClickableCell
                        value={project.noOfTaxInvoice}
                        color="purple"
                        onClick={() =>
                          navigate(`/projects/${project.projectId}/tax-invoice`)
                        }
                      />
                      ...
                      <ClickableCell
                        value={project.noOfInvoiceBooked}
                        color="orange"
                        onClick={() =>
                          navigate(
                            `/projects/${project.projectId}/invoice-booked`,
                          )
                        }
                      />
