...
export default function ProjectsPage({ onNavigate, filterProjectType, filterCount, onClearFilter }: Props) {
  ...
  const filteredProjects = useMemo(() => {
    // When user clicked a project type from Manager page,
    // generate exactly `filterCount` records for that type.
    let data: typeof projectDetails;
    if (filterProjectType) {
      data = filterCount && filterCount > 0
        ? generateProjectsForType(filterProjectType, filterCount)
        : projectDetails.filter((p) => p.projectType === filterProjectType);
    } else {
      data = projectDetails;
    }
  ...
  }, [search, filterProjectType, filterCount, columnFilters]);
  ...
  useMemo(() => { setCurrentPage(1); }, [search, filterProjectType, columnFilters]);
      ...
      {filterProjectType && (
        <div className="flex items-center gap-3 rounded-2xl border border-primary-200 bg-primary-50 px-5 py-3 text-sm">
          ...
          <span className="text-primary-700">
            ...
            <span className="ml-1 inline-flex items-center rounded-lg bg-primary-600 px-2.5 py-0.5 text-xs font-bold text-white">{filterProjectType}</span>
          </span>
          <button
            type="button"
            onClick={onClearFilter}
              ...
              {paginatedProjects.map((project) => (
                ...
                >
                  ...
                  <ClickableCell
                    ...
                    color="yellow"
                    onClick={() => onNavigate("invoiceBooked")}
                  />
