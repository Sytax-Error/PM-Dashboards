...
export default function InvoiceReceivedPage() {
      ...
      const fetchInvoices = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `${API_BASE_URL}/api/pm/billdesk/project/${projectId}`,
            {
              headers: {
  ...
  const projects = useMemo(
    () => [...new Set(invoiceData.map((d) => d.projectNumber))],
    [invoiceData],
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">
              Project Wise Exp Invoice Details Received at Bill Desk
            </h2>
            <p className="text-sm text-slate-500 mt-0.5">
              Invoices received at Billdesk
            </p>
          </div>
        </div>
      ...
      {!error && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="pm-card rounded-2xl p-5">
              <p className="text-sm text-slate-500">Payment Done</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">
                {
                  filteredData.filter((d) => d.invoiceStatus === "Payment Done")
                    .length
                }
              </p>
            </div>
            ...
            {paginatedData.length === 0 && !loading ? (
              <div className="p-20 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  There are no invoice records matching the current filters.
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-blue-700 text-white">
                      ...
                      <th className="px-4 py-3.5 text-left font-bold border-l border-white/20 text-xs">
                        Created Date
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {paginatedData.map((d) => (
                      <tr
                        key={d.srNo}
                        className={`hover:bg-blue-50/40 transition-colors ${d.srNo % 2 === 0 ? "bg-white" : "bg-slate-50/30"}`}
                      >
                        ...
                        <td className="px-4 py-3.5 text-right border-l border-slate-200/60 tabular-nums font-medium text-blue-700">
                        ...
                        <td className="px-4 py-3.5 text-right border-l border-slate-200/60 tabular-nums text-slate-700">
                          {d.bookAmount > 0
                            ? formatCurrency(d.bookAmount)
                            : "-"}
                        </td>
                        <td className="px-4 py-3.5 text-right border-l border-slate-200/60 tabular-nums font-medium text-green-700">
                          {d.amountPaid > 0
                        ...
                        <td className="px-4 py-3.5 text-center border-l border-slate-200/60">
                          <span
                            className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${d.invoiceStatus === "Payment Done" ? "bg-green-100 text-green-700" : d.invoiceStatus === "Pending With Provision" ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}
                          >
                            {d.invoiceStatus}
                          </span>
                        </td>
                        <td className="px-4 py-3.5 text-slate-500 text-[10px] border-l border-slate-200/60 leading-tight">
                          {d.createdDate}
                        </td>
                      </tr>
                    ))}
